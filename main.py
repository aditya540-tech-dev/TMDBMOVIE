...
TFIDF_MATRIX_PATH = os.path.join(BASE_DIR, "tfidf_maatrix.pkl")
TFIDF_PATH = os.path.join(BASE_DIR, "tfidf.pkl")

df: Optional[pd.DataFrame] = None
indices_obj: Any = None
tfidf_maatrix: Any = None
tfidf_obj: Any = None
...
def tfidf_recommend_titles(
    ...
    """
    global df, tfidf_maatrix
    if df is None or tfidf_maatrix is None:
        raise HTTPException(status_code=500, detail="TF-IDF resources not loaded")

    idx = get_local_idx_by_title(query_title)

    # query vector
    qv = tfidf_maatrix[idx]
    scores = (tfidf_maatrix @ qv.T).toarray().ravel()
...
def load_pickles():
    global df, indices_obj, tfidf_maatrix, tfidf_obj, TITLE_TO_IDX
